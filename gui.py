...
def phase_vocoder(
    ...
    freq_axis = 2 * np.pi * torch.arange(audio_len // 2 + 1).to(prev_audio.device) + phase_diff
    ...
    result = (
        ...
        + torch.sum(amp_sum * torch.cos(freq_axis * time_axis + prev_phase), dim=-1) * mix_window / audio_len  # 相位对齐部分
    )
...
class HarvestPitchProcess(Process):
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            task_idx, audio_data, result_dict, total_processes, timestamp = self.input_queue.get()
...
class VoiceConversionGUIConfig:
    ...
    def __init__(self):
        ...
        self.sr_mode: str = "sr_model"     # 采样率模式（sr_model:模型采样率；sr_device:设备采样率）
        ...
        # 硬件与性能参数
        self.harvest_cpu_count: int = min(max_harvest_cpu, 4)  # Harvest进程数（1~max_harvest_cpu）
        self.f0_extract_method: str = "fcpe"  # 基频提取方法（pm/dio/harvest/crepe/rmvpe/fcpe）
...
class VoiceConversionGUIMain:
    ...
    def init_audio_buffers(self):
        ...
        self.input_audio_buf: torch.Tensor = torch.tensor([], dtype=torch.float32)  # 主输入缓冲区
        self.denoised_audio_buf: torch.Tensor = torch.tensor([], dtype=torch.float32)  # 降噪后输入缓冲区
        self.resampled_audio_buf: torch.Tensor = torch.tensor([], dtype=torch.float32)  # 16k重采样缓冲区（模型输入用）
        self.output_audio_buf: torch.Tensor = torch.tensor([], dtype=torch.float32)  # 输出缓冲区（降噪用）
        
        # 辅助缓冲区
        self.rms_detect_buf: np.ndarray = np.array([], dtype=np.float32)  # RMS静音检测缓冲区
        self.sola_buffer: torch.Tensor = torch.tensor([], dtype=torch.float32)  # SOLA拼接缓冲区
        self.denoise_buffer: torch.Tensor = torch.tensor([], dtype=torch.float32)  # 降噪历史缓冲区
        
        # 窗口函数（后续根据SOLA缓冲区大小动态生成）
        self.fade_in_window: torch.Tensor = torch.tensor([], dtype=torch.float32)  # 淡入窗口
        self.fade_out_window: torch.Tensor = torch.tensor([], dtype=torch.float32)  # 淡出窗口
    ...
    def check_necessary_assets(self):
        ...
        if not check_all_assets(update=self.app_config.update):
            if self.app_config.update:
                ...
                print_with_format("[Asset Check] 缺失必要资源，请启用更新模式重新运行！")
                sys.exit(1)
    ...
    def _get_default_config(self) -> dict:
        ...
        default_input_dev_name = self.input_device_list[self.input_device_index_list.index(default_input_dev_idx)]
        default_output_dev_name = self.output_device_list[self.output_device_index_list.index(default_output_dev_idx)]
    ...
    def _validate_device_config(self, config: dict) -> dict:
        ...
        if config.get("sg_input_device") not in self.input_device_list:
            ...
            config["sg_input_device"] = self.input_device_list[self.input_device_index_list.index(default_input_idx)]
            print_with_format(f"[Config Validate] 输入设备无效，重置为：{config['sg_input_device']}")
        
        # 验证输出设备
        if config.get("sg_output_device") not in self.output_device_list:
            default_output_idx = sd.default.device[1]
            config["sg_output_device"] = self.output_device_list[self.output_device_index_list.index(default_output_idx)]
            print_with_format(f"[Config Validate] 输出设备无效，重置为：{config['sg_output_device']}")
    ...
    def update_audio_devices(self, hostapi_name: str = None):
        ...
        self.input_device_list = [
            dev["name"] for dev in all_devices
        ...
        self.input_device_index_list = [
            dev["index"] for dev in all_devices
            if dev["max_input_channels"] > 0 and dev["hostapi_name"] == target_api
        ]
        ...
        print_with_format(f"[Device Update] 已更新{target_api}下的设备列表：输入设备{len(self.input_device_list)}个，输出设备{len(self.output_device_list)}个")
            ...
            layout=[
                ...
                [
                    ...
                    sg.Combo(
                        ...
                        size=(45, 1),
                        expand_x=True,
                        tooltip=i18n("Select the audio input device (e.g., microphone)")
                    )
                ],
                [
                ...
                [
                    ...
                    sg.Combo(
                        ...
                        enable_events=True,
                        size=(8, 1),
                        tooltip=i18n("Select the target sampling rate for audio processing")
                    ),
                    sg.Text(key="current_sr_display", tooltip=i18n("Currently active sampling rate"))
                ]
            ],
            expand_x=True
        )
            ...
            layout=[
                [
                    ...
                    sg.Slider(
                        ...
                        enable_events=True,
                        expand_x=True,
                        tooltip=i18n("Audio below this level is treated as silence (-60 to 0 dB)")
                    )
                ],
                [
                    ...
                    sg.Slider(
                        ...
                        enable_events=True,
                        expand_x=True,
                        tooltip=i18n("Shift pitch by this number of semitones (-24 to 24)")
                    )
                ],
                [
                ...
                [
                    ...
                    sg.Slider(
                        ...
                        enable_events=True,
                        expand_x=True,
                        tooltip=i18n("Match output loudness to input (0.0 = full match)")
                    )
                ],
                [
            ...
            layout=[
                [
                    ...
                    sg.Slider(
                        ...
                        enable_events=True,
                        expand_x=True,
                        tooltip=i18n("Duration of each audio processing block (lower = more responsive)")
                    )
                ],
                [
                    ...
                    sg.Slider(
                        ...
                        resolution=1,
                        orientation="h",
                        default_value=saved_config.get("n_cpu", min(max_harvest_cpu, 4)),
                        enable_events=True,
                        expand_x=True,
                ...
                [
                    ...
                    sg.Slider(
                        ...
                        enable_events=True,
                        expand_x=True,
                        tooltip=i18n("Additional time for model inference to prevent clipping")
                    )
                ],
                [
                    sg.Checkbox(
        ...
        while True:
            try:
                ...
                # 参数更新事件（滑块、复选框等）
                    ...
                    "silence_threshold", "pitch_shift", "formant_shift", 
                    "index_ratio", "rms_mix_ratio", "f0_pm", "f0_dio", 
                    "f0_harvest", "f0_crepe", "f0_rmvpe", "f0_fcpe",
                    "enable_input_denoise", "enable_output_denoise", 
                    "enable_phase_vocoder", "mode_voice_convert", "mode_input_monitor"
                ]:
    ...
    def handle_samplerate_change(self, values):
        ...
        try:
            ...
            print_with_format(f"[Samplerate] Changed to {self.gui_config.target_samplerate} Hz")
        except Exception:
    ...
    def handle_device_event(self, values):
        ...
        try:
            ...
            # 更新GUI控件
            self.window["hostapi_name"].update(values=self.hostapi_list, value=self.gui_config.hostapi_name)
            
            # 重置输入设备（如果当前设备不在新列表中）
            if self.gui_config.input_device_name not in self.input_device_list and self.input_device_list:
                self.gui_config.input_device_name = self.input_device_list[0]
            self.window["input_device"].update(values=self.input_device_list, value=self.gui_config.input_device_name)
            
            # 重置输出设备（如果当前设备不在新列表中）
            if self.gui_config.output_device_name not in self.output_device_list and self.output_device_list:
                self.gui_config.output_device_name = self.output_device_list[0]
            self.window["output_device"].update(values=self.output_device_list, value=self.gui_config.output_device_name)
    ...
    def handle_start_conversion(self, values):
        ...
        try:
            ...
            # 更新UI显示的采样率和延迟
            self.window["current_sr_display"].update(f"{self.gui_config.target_samplerate} Hz")
            self.window["delay_time_display"].update(int(np.round(self.processing_delay * 1000)))
    ...
    def set_audio_devices(self, input_device_name: str, output_device_name: str):
        ...
        try:
            # 获取设备索引
            input_idx = self.input_device_index_list[self.input_device_list.index(input_device_name)]
            output_idx = self.output_device_index_list[self.output_device_list.index(output_device_name)]
            
            # 设置默认设备
            sd.default.device[0] = input_idx
            sd.default.device[1] = output_idx
            
            print_with_format(f"[Device Setup] Input: {input_device_name} (Index: {input_idx})")
            print_with_format(f"[Device Setup] Output: {output_device_name} (Index: {output_idx})")
    ...
    def verify_samplerate_compatibility(self):
        ...
        try:
            ...
            sg.popup(
                i18n(f"Selected samplerate {target_sr} Hz not supported.\nFalling back to {fallback_sr} Hz")
            )
    ...
    def calculate_audio_parameters(self):
        ...
        self.block_frame = int(
            np.round(self.gui_config.block_duration * self.gui_config.target_samplerate / self.zc_step)
        ) * self.zc_step
        ...
        self.crossfade_frame = int(
            np.round(self.gui_config.crossfade_duration * self.gui_config.target_samplerate / self.zc_step)
        ) * self.zc_step
        ...
        self.extra_frame = int(
            np.round(self.gui_config.extra_infer_duration * self.gui_config.target_samplerate / self.zc_step)
        ) * self.zc_step
        
        # 模型推理参数
        self.skip_head_frame = self.extra_frame // self.zc_step
        self.return_length_frame = (self.block_frame + self.sola_buffer_frame + self.sola_search_frame) // self.zc_step
    ...
    def initialize_audio_buffers(self):
        ...
        buffer_size = self.extra_frame + self.crossfade_frame + self.sola_search_frame + self.block_frame
    ...
    def start_audio_stream(self):
        ...
        if not flag_vc:
            ...
            # 连接共享内存
            self.input_shared_mem = shared_memory.SharedMemory(name=self.audio_stream.get_in_mem_name())
            self.output_shared_mem = shared_memory.SharedMemory(name=self.audio_stream.get_out_mem_name())
            ...
            (self.input_write_ptr, 
             ...
             self.playback_ptr, 
             self.input_event, 
             self.stop_event) = self.audio_stream.get_ptrs_and_events()
            ...
            # 启动音频处理线程
            import threading
            audio_thread = threading.Thread(target=self.audio_processing_loop, daemon=True)
            audio_thread.start()
    ...
    def audio_processing_loop(self):
        """音频实时处理主循环"""
        while flag_vc:
            self.process_audio_block(self.block_frame << 1)  # buf_size = 2 * block_frame
    ...
    def process_audio_block(self, buf_size: int):
        ...
        try:
            ...
            # 更新主输入缓冲区
            self.input_audio_buf[:-self.block_frame] = self.input_audio_buf[self.block_frame:].clone()
            self.input_audio_buf[-processed_data.shape[0]:] = torch.from_numpy(processed_data).to(self.app_config.device)
    ...
    def apply_silence_detection(self, audio_data: np.ndarray) -> np.ndarray:
        ...
        if self.gui_config.silence_threshold > -60:
            ...
            # 检测静音帧并归零
            silence_mask = librosa.amplitude_to_db(rms, ref=1.0)[0] < self.gui_config.silence_threshold
    ...
    def process_input_audio(self):
        ...
        if self.gui_config.enable_input_denoise:
            # 更新降噪缓冲区
            self.denoised_audio_buf[:-self.block_frame] = self.denoised_audio_buf[self.block_frame:].clone()
            
            # 提取需要降噪的音频片段
            audio_segment = self.input_audio_buf[-self.sola_buffer_frame - self.block_frame:]
            
            # 应用降噪
            denoised_segment = self.torch_gate(
                audio_segment.unsqueeze(0), 
                self.input_audio_buf.unsqueeze(0)
            ).squeeze(0)
            
            # 应用淡入窗口，与历史数据平滑过渡
            denoised_segment[:self.sola_buffer_frame] *= self.fade_in_window
            denoised_segment[:self.sola_buffer_frame] += self.denoise_buffer * self.fade_out_window
            
            # 更新缓冲区
            self.denoised_audio_buf[-self.block_frame:] = denoised_segment[:self.block_frame]
            self.denoise_buffer[:] = denoised_segment[self.block_frame:]
            
            # 重采样到16k（模型输入要求）
            self.resampled_audio_buf[-self.block_frame_16k - 160:] = self.resampler_to_16k(
                self.denoised_audio_buf[-self.block_frame - 2 * self.zc_step:]
            )[160:]
        else:
            # 无降噪时直接重采样
            self.resampled_audio_buf[-160 * (self.block_frame // self.zc_step + 1):] = (
                self.resampler_to_16k(self.input_audio_buf[-self.block_frame - 2 * self.zc_step:])[160:]
            )
    ...
    def apply_output_denoising(self, audio: torch.Tensor) -> torch.Tensor:
        """对输出音频应用降噪处理"""
        self.output_audio_buf[:-self.block_frame] = self.output_audio_buf[self.block_frame:].clone()
        self.output_audio_buf[-self.block_frame:] = audio[-self.block_frame:]
        
        return self.torch_gate(
            audio.unsqueeze(0),
            self.output_audio_buf.unsqueeze(0)
        ).squeeze(0)
    ...
    def match_volume(self, output_audio: torch.Tensor) -> torch.Tensor:
        ...
        out_rms = torch.max(out_rms, torch.tensor(1e-3, device=self.app_config.device))  # 避免除零
        
        # 应用音量匹配
        return output_audio * torch.pow(ref_rms / out_rms, torch.tensor(1 - self.gui_config.rms_mix_ratio))

    def stitch_audio_blocks(self, audio_block: torch.Tensor) -> torch.Tensor:
        """使用SOLA算法或相位声码器拼接音频块，确保平滑过渡"""
        # 计算与历史缓冲区的互相关，找到最佳对齐位置
        conv_input = audio_block[None, None, :self.sola_buffer_frame + self.sola_search_frame]
        cor_nom = F.conv1d(conv_input, self.sola_buffer[None, None, :])
        cor_den = torch.sqrt(
            F.conv1d(conv_input**2, torch.ones(1, 1, self.sola_buffer_frame, device=self.app_config.device)) + 1e-8
        )
        ...
        # 应用淡入淡出或相位声码器
        if "privateuseone" in str(self.app_config.device) or not self.gui_config.enable_phase_vocoder:
            # 普通淡入淡出
            audio_block[:self.sola_buffer_frame] *= self.fade_in_window
            audio_block[:self.sola_buffer_frame] += self.sola_buffer * self.fade_out_window
        else:
            # 相位声码器（更高质量）
        ...
        # 更新SOLA缓冲区
        self.sola_buffer[:] = audio_block[self.block_frame:self.block_frame + self.sola_buffer_frame]
        ...
        try:
            if event == "silence_threshold":
                ...
                if self.audio_stream is not None:
                    ...
                    self.processing_delay += delay_adjust if values["enable_input_denoise"] else -delay_adjust
                    self.window["delay_time_display"].update(int(np.round(self.processing_delay * 1000)))
    ...
    def get_device_channel_count(self) -> int:
        ...
        input_channels = sd.query_devices(device=sd.default.device[0])["max_input_channels"]
        output_channels = sd.query_devices(device=sd.default.device[1])["max_output_channels"]
