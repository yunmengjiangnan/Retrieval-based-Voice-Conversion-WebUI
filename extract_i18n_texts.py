...
if not os.path.exists(en_json_path):
    ...
    if new_entries > 0:
        with open(en_json_path, 'w', encoding='utf-8') as file:
            json.dump(en_json_data, file, ensure_ascii=False, indent=4)
        print(f"Updated English translation file: {en_json_path} (added {new_entries} new entries)")
    else:
...
if not os.path.exists(zh_json_path):
    ...
    if new_entries > 0:
        with open(zh_json_path, 'w', encoding='utf-8') as file:
            json.dump(zh_json_data, file, ensure_ascii=False, indent=4)
        print(f"Updated Chinese translation file: {zh_json_path} (added {new_entries} new entries)")
        print(f"Please translate the {new_entries} new entries in the Chinese JSON file")
    else:
